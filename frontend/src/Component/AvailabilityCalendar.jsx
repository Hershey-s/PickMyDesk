...
const AvailabilityCalendar = ({ workspace, onTimeSelect }) => {
  ...
  const calendarData = getCalendarData(currentDate.getFullYear(), currentDate.getMonth());
  ...
  useEffect(() => {
    if (selectedDate && workspace) {
      ...
      const updatedSlots = slots.map(slot => ({
        ...slot,
        available: !isTimeSlotBooked(bookings, formatDateForAPI(selectedDate), slot.time)
      }));
  ...
  const getDayClasses = (dateObj) => {
    ...
    } else if (selectedDate && dateObj.date.toDateString() === selectedDate.toDateString()) {
    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          {selectedTime && (
            <div className="mt-4 p-3 bg-purple-50 rounded-lg">
              <div className="text-sm text-purple-700">
                Selected: {selectedDate.toLocaleDateString()} at {selectedTime.displayTime}
              </div>
            </div>
          )}
        </div>
      ...
      {!selectedDate && (
        <div className="text-center py-8">
          ...
          <div className="text-sm text-gray-500">Select a date to view available time slots</div>
        </div>
      )}
    </div>
  );
